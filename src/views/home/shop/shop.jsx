...
function Shop() {
  ...
  const [selectedEnvelop, setSelectedEnvelop] = useState([
    null,
    null,
    null,
    null,
  ...
  function selectingEnvelop(i) {
    ...
    setRandomCards((prev) =>
      prev.map((envelop, index) => (index === i ? selected : envelop)),
    );
          ...
          {selectedEnvelop.map((isSelected, i) => (
            ...
            >
              <img
                ...
                className="envelops__container-img"
              ...
              <ul className="envelops__container-cards">
                {randomCards[i].map((card) => (
                  <li
                    key={card.id}
                    className={
                      `shopCards myCards__container__list__item card-type--` +
                      card.tipo[0]
                    }
                  >
                    <div className="myCards__container__list__item-nameContainer">
                      <p className="myCards__container__list__item-name">
                        {card.name}
                      </p>
                      <p className="myCards__container__list__item-PS">
                        <span>PS</span>
                    ...
                    <div
                      className={
                        `myCards__container__list__item-imgContainer myCards__container__list__item-imgContainer--` +
                        card.tipo[0]
                      }
                    >
                      <img
                        className="myCards__container__list__item-image"
                        src={card.isShiny ? card.imageShiny : card.image}
                        alt={card.name + ` image`}
                      />
                      <img
                        className="myCards__container__list__item-back"
                      ...
                      <div className="cards-typeContainer">
                        {card.tipo.map((type, index) => (
                          <p
                            key={index}
                            className={`cards-type pokedex-type--` + type}
                          >
                            {type}
                          </p>
                        ))}
                    ...
                    <p className="myCards__container__list__item-description">
                      {card.description}
                    </p>
                    <div className="myCards__container__list__item-habilityContainer">
                      <p className="myCards__container__list__item-hability">
                        {card.Habilidad} <span>?</span>
                      </p>
                      {(() => {
                        const attackFind = attack.find(
                          (a) => a.Habilidad === card.Habilidad,
                        );
                        return attackFind ? (
                          <p className="myCards__container__list__item-PH">
                            <span>PH:</span> {attackFind.PH}
                          </p>
                        ) : (
